...
class TestCapOutputBytesUnit:
    ...
    def test_under_cap_returned_unchanged(self):
        small = "hello world\n" * 10
        assert _cap_output_bytes(small, 262144) is small or _cap_output_bytes(small, 262144) == small
    ...
    def test_head_is_roughly_60_percent_tail_roughly_30_percent(self):
        ...
        before_marker, _, after_marker = capped.partition("[...")
        # head should be close to 60% of the cap
        assert abs(len(before_marker.encode("utf-8")) - int(cap * 0.6)) < 16
        # tail (after the marker line) should be close to 30% of the cap
        tail_text = after_marker.split("...]", 1)[1] if "...]" in after_marker else ""
        assert abs(len(tail_text.encode("utf-8")) - int(cap * 0.3)) < 16
    ...
    def test_multibyte_boundary_no_corruption(self):
        ...
        # Must round-trip cleanly: no dangling/partial multi-byte sequences.
        decoded_again = encoded.decode("utf-8")
        assert decoded_again == capped
    ...
    def test_elided_count_matches_dropped_bytes(self):
        ...
        head_bytes = int(cap * 0.6)
        tail_bytes = int(cap * 0.3)
