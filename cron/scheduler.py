...
def _deliver_skip_notices(events: list, adapters=None, loop=None) -> int:
    """Deliver notices for recurring runs skipped via ``misfire_deadline_seconds``.

    ``cron/jobs.py:_get_due_jobs_locked`` returns skips (jobs past their
    opt-in misfire deadline, measured from the scheduled slot independent of
    the grace window — see #33315 for why the *default* behavior is still
    grace-gated fire-once) alongside the due list instead of delivering
    inline, because that function runs under the cross-process
    ``_jobs_lock()`` and must not do network I/O while holding it. This
    delivers each returned skip event through the normal ``_deliver_result``
    path, same as a failed run's summary — just called from ``tick()`` right
    after ``get_due_jobs(include_skips=True)`` returns, once the lock is no
    longer held.
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        due_jobs, skipped_stale = get_due_jobs(include_skips=True)

        # Deliver notices for stale runs skipped during the scan above
        ...
        _deliver_skip_notices(skipped_stale, adapters=adapters, loop=loop)
