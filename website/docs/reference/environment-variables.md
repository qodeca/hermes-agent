...
| `GATEWAY_ALLOW_ALL_USERS` | Allow all users without allowlists (`true`/`false`, default: `false`). Deprecated — prefer per-platform `<PLATFORM>_ALLOW_ALL_USERS`; any active allow-all logs a startup WARNING and (when `alerts.deliver` is set) an operator alert. |
...
| `HERMES_CRON_TIMEOUT` | Inactivity timeout for cron job agent runs in seconds (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. Pairs with the wall-clock cap `HERMES_CRON_MAX_RUNTIME` below — whichever trips first ends the run. |
| `HERMES_CRON_MAX_RUNTIME` | Wall-clock runtime cap for a cron run in seconds (default: `3600`). Bounds total runtime **regardless of activity** — the counterpart to the inactivity-based `HERMES_CRON_TIMEOUT`. Set to `0` for unlimited. Wins over `cron.max_runtime_seconds` in `config.yaml`. |
| `HERMES_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `3600`). Bounds the script only — skill/agent jobs use the separate `HERMES_CRON_TIMEOUT` / `HERMES_CRON_MAX_RUNTIME` limits. Also configurable via `cron.script_timeout_seconds` in `config.yaml`. |
| `HERMES_CRON_MAX_PARALLEL` | Max cron jobs run in parallel per tick (default: `4`). |
| `HERMES_MACHINE_ID` | Identifies this machine/instance for cron running-marker reap scoping (default: derived from `host:pid`). At startup, cron reconciliation reaps orphaned run markers that are TTL-expired **or** share this identifier's host prefix. Set distinct values when running more than one Hermes instance on the same host, so restarting one instance doesn't reap the other's live runs. |
